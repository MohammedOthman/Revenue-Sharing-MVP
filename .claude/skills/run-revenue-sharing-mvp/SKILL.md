...
The "app" is a **static UI prototype**: **57** standalone HTML screens of Partner
Revenue OS (the MVP→V3 journey) under `prototype/screens/` — 46 from the original
Stitch export plus 11 generated by `gen-screens.mjs` to cover all 22 phases of the
End-to-End Business Workflow (see `workflow.mjs`).
...
```bash
cd prototype
node build.mjs     # -> "Built 57 interactive screens + 5 design-only frames -> dist/"
```

The 11 generated gap screens are committed under `screens/`, so a plain
`node build.mjs` already includes them. Only re-run the generator if you change
`workflow.mjs` or the screen specs:

```bash
cd prototype
node gen-screens.mjs && node build.mjs && node gen-thumbs.mjs && node build.mjs
```
...
- **Deep-links follow the product journey.** `build.mjs` also injects, on **55
  ...
  Settlement, registry row → Partner Profile). Matching is by exact icon-stripped
  control text / DOM order, so only the named controls are wired. To add/adjust, edit
