...
The deployable product lives in [`revenue-share-platform/`](revenue-share-platform/README.md).
It is a self-hosted React and Express application backed by PostgreSQL, with no
external application-backend dependency. Production entry points are provided
at the repository root:

- `Dockerfile` — reproducible production image.
- `package.json` — automatic Node-platform build and start commands.
- `DEPLOYMENT.md` — environment, backup, monitoring, smoke-test, and rollback runbook.
- `compose.yaml` — complete local environment for application testing.
