...
| **Market analysis & timing** | [`Reven_Economic_Trends_and_Why_Now.md`](Reven_Economic_Trends_and_Why_Now.md) — the 13 economic trends expanded (rationale · why now · strategic + operational implications · falsifiers), the **multi-sector field map** (every industry with revenue-share economics, not ICT), the dated compliance calendar, and product lenses. **Carries the live ZATCA Wave 25 correction (1 Feb 2027) that supersedes the expired Wave 24 hook.** Companions: `Reven_PERM_Category_Deep_Dive.md`, `Saudi_Value_Pool_and_ICP_Strategic_Analysis.md`, the `Market Analysis — Growth Drivers` PDF set |
| **Trends, separated** | [`trends/`](trends/) — the same thirteen trends as **standalone files**, one per trend, each self-contained (claim · rationale with its own evidence · dated why-now · strategic and operational implications · falsifier · own sources). Use these when a trend needs to travel alone — a slide, a memo, a section. [`trends/README.md`](trends/README.md) carries the index, the moat/mechanism/market classification, and the recommended lead order |
| **Lenses, separated** | [`lenses/`](lenses/) — the twelve analytical frames as **standalone files** (the frame · rationale · what it changes · the argument against it · cross-links). Judgment rather than evidence — read differently from `trends/`. [`lenses/README.md`](lenses/README.md) indexes them by type and names the three to read if you read only three |
| **Market sizing (investor-facing)** | [`Reven_Market_Sizing_TAM_SAM_SOM.md`](Reven_Market_Sizing_TAM_SAM_SOM.md) — **the canonical TAM/SAM/SOM**, written for generalist investors. Market ceiling, per-account value ceiling, the ~30× expansion multiple, per-industry allocation, five-year SOM build, and the anti-inflation rules (no GMV, no take-rate, no multiplying Saudi by six). Method from `Reven_Market_Architecture_Adoption_and_Economic_Potential_Report.docx` §12 |
