...
- Rental: Ejar registered >1.5M contracts in Jan–Sep 2024 (REGA — HIGH) but publishes no contract values, so the rental commission pool (2.5% of first-year rent, typically tenant-paid — MED) is not computable (§9).
...
| E. Compliance add-ons | ZATCA e-invoice generation for commissions (self-billing), WHT computation/filing, IFRS-17 commission subledger, PDPL-resident deployment | per-entity SAR 20–80k/yr | SAR 20–160M on 1,000–2,000 entities | ZATCA Wave 24 completion (Jun 2026) makes this table-stakes |
...
| Dealer F&I commissions (banks → auto dealers) | Possibly a SAR 1–3B invisible pool | Bank annual-report "fees & commissions paid" notes (Al Rajhi, SNB, Emkan, Nayifat) read directly; or SAMA finance-company disclosure requests |
...
| Ejar rental contract values | Rental commission pool (2.5% of first-year rent × >1.5M contracts/yr) is not computable | REGA/Ejar data releases; average-rent series from GASTAT RPI would bound it |
