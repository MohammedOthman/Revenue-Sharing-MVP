...
- **Regulatory boundary:** through MVP–V2 the product is a SaaS workflow and evidence layer, not a regulated financial service — it calculates eligibility and produces statements/evidence but does not execute payments, move or hold funds, or act as a payment processor. Payout execution is V3-only (see the Integration Layer manual §20 and `README.md` §2) and is additionally gated behind a documented licensing review (PayFac / money-transmitter / SAMA payments).
