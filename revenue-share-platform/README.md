...
- First registered user = admin (bootstrap); registration then locks down; admins create accounts (role from the request body is ignored for non-admins).
...
| Area | Routes |
|---|---|
| Auth | `POST /auth/register`, `POST /auth/login`, `GET /auth/profile`, admin: `GET/PUT/DELETE /auth/users[/:id]` |
...
| Revenue | `GET/POST /revenue`, `GET/PUT/DELETE /revenue/:id`, `POST /revenue/:id/process-payment`, `GET /revenue/pending-payments`, `GET /revenue/contract/:contractId`, `GET /revenue/stats`, `GET /revenue/trends` |
...
| Dashboard | `GET /dashboard/overview`, `/revenue-trends`, `/top-partners`, `/recent-activity`, `/analytics/contract-status` |
